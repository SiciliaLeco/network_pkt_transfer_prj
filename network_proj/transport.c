...
void send_udp(uint src_port, uint dst_port, uchar* payload, uint len){
	...
	char buf[len + 8];
	uchar hdr[4];
	hdr[0] = header.src_port = src_port;	printf("src port:%d\n", hdr[0]);
	hdr[1] = header.dest_port = dst_port;	printf("dst port:%d\n", hdr[1]);
	hdr[2] = header.checksum = 0;			printf("udp checksum:%d\n", hdr[2]);
	hdr[3] = header.len = len + 8;  		printf("udp length:%d\n", hdr[3]);
...
void udp_send_to_ip(char* packet, uint len){
	/*
